...
1.  **bidirectional-longform30023.js** 
	- Nostr ➡️ Hive - Listens *only* for kind 30023 (long form) Nostr notes to bridge over to Hive. Kind 1 short notes are ignored.
	- Hive ➡️ Nostr - Any Hive post over 280 characters gets truncated as a kind 1 (short form) Nostr note (with a link back to the Hive post).

  

 2. **bidirectional-bridge.js** 
    - Nostr➡️Hive - This script listens for both kind 1 (short form) and kind 30023 (long form) Nostr notes to bridge over to Hive.
	- Hive ➡️ Nostr - Same as above (280+ is truncated).
...
1. If you post frequently on Nostr (more than 2 times per day?), the bidirectional-longform30023.js script is likely best. Only long form notes will bridge over. 
   - For newcomers to Hive, I would start with this one.
