...
const relays = [
  ...
  'wss://nostr.wine',
  'wss://purplepag.es',
  'wss://relay.nostr.band', // Added for kind 30023 reliability
...
function generateTitle(content, kind, tags) {
  ...
  return cleanContent.substring(0, 57) + '...';
}
...
function queueNostrToHive(event) {
  if (event.content.includes('Originally posted on Hive at https://hive.blog')) {
    console.log(`[Nostr→Hive] ⏭️ Skipping Hive-originated note: "${event.content.substring(0, 30)}..."`);
    return;
  }
  ...
  if (!nostrToHiveQueue.some(item => item.content === event.content)) {
    ...
    console.log(`[Nostr→Hive] ⏭️ Skipping duplicate content: "${event.content.substring(0, 30)}..."`);
  }
}
...
function queueHiveToNostr(post) {
  // Prevent loop by skipping Nostr-originated or truncated posts
  const bodyLower = post.body.toLowerCase();
  if (bodyLower.includes('originated on [nostr]') || bodyLower.includes('originated on nostr') || bodyLower.includes('read full post below')) {
    console.log(`[Hive→Nostr] ⏭️ Skipping Nostr-originated or truncated post: "${post.title}" (Permlink: ${post.permlink})`);
    return;
  }
  ...
  if (content.length > 280) {
    ...
    const suffix = `... read full post below:\n${hiveLink}`;
...
async function start() {
  ...
  console.log('[Bridge] 🎧 Starting bidirectional bridge...');
  await listenToNostr();
  await pollHive();
  keepAlive();
}
