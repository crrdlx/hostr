// bidirectional-bridge.cjs v0.1.62 Snaps+Waves+Longform
...
console.log('Debug: Starting bidirectional-bridge.cjs v0.1.62');
...
async function postToHive(content, eventId, containerPermlink, frontEnd) {
  ...
  const body = `${content}\n\nBridged via Hostr v${VERSION}(br) at https://hostr-home.vercel.app, view original on [Nostr](${nostrLink}).`;
...
async function postLongformToHive(content, eventId, tags) {
  ...
  const nostrLink = createNostrLink(eventId);
  const body = `${content}\n\nView the original article on [Nostr](${nostrLink})\nBridged via Hostr v${VERSION}(lf)`;
  const jsonMetadata = JSON.stringify({
    tags: ['hostr', 'longform'],
...
async function pollHive() {
  try {
    ...
    for (const post of sortedPosts) {
      ...
      // Check for .hostr keyword to skip the post
...
function queueHiveToNostr(post) {
  ...
  const nonTruncatedFooter = `\n\nView original on Hive at ${hiveLink}`;
  let jsonMetadata;
  try {
...
// bidirectional-bridge.cjs v0.1.62 Snaps+Waves+Longform
// Added .hostr ignore command for n2h both short and long form, also h2n